export class Particle {
    ...
    draw(ctx: CanvasRenderingContext2D, gravity: boolean) {
        ...
        let gradient = ctx.createRadialGradient(drawX, drawY, 0, drawX, drawY, this.radius);
    ...
    isCollided(particle: Particle): boolean {
        ...
        if(this.x + this.radius + particle.radius > particle.x &&
           ...
           this.y < particle.y + this.radius + particle. radius) {
            ...
            let distance = Math.sqrt(Math.pow(this.x - particle.x, 2) + Math.pow(this.y - particle.y, 2));
    ...
    private getNextDistanceBetweenParticles(particle: Particle): number {
        let x1 = this.x + (this.vx * 0.01);
        let y1 = this.y + (this.vy * 0.01);
        let x2 = particle.x + (particle.vx * 0.01);
        let y2 = particle.y + (particle.vy * 0.01);
